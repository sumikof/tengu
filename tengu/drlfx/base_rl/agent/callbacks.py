...
class MovieLogger(rl.callbacks.Callback):
    ...
    def save(self,
        ...
        plt.axis('off')
        ani = matplotlib.animation.FuncAnimation(plt.gcf(), self._plot, frames=end_frame - start_frame,
                                                 interval=interval)
...
class ConvLayerView(rl.callbacks.Callback):
    ...
    def save(self, grad_cam_layers, add_adv_layer=False, add_val_layer=False, start_frame=0, end_frame=0, gifname="",
             mp4name="", interval=200, fps=30):
...
class TrainLogger(keras.callbacks.Callback):
    ...
    def _record(self, logs):
        ...
        if self.test_agent is not None:
            ...
            history = self.test_agent.test(self.test_env, nb_episodes=self.test_episodes, visualize=False,
                                           verbose=False)
